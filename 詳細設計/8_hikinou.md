...
**ログ出力**
- INFO レベルで業務処理を記録し、WARN・ERROR は障害調査の起点となるよう適切にログ出力する。
- 開発・ステージング環境ではDEBUGログを有効化し、詳細調査が必要な際に参照可能とする。

**ログローテーション**
- ログファイルはファイルサイズ（例:100MB）または日次でローテーションし、最大世代数を設定して自動的に古いログを削除する。
- ログ保存期間はシステム運用ポリシー（例:90日）に従い管理する。

**監視項目**
- サーバー資源（CPU負荷、メモリ使用率、ディスク使用率）を監視し、閾値超過時には自動的にアラート通知する。
- アプリケーションログにおけるERROR件数が一定時間内に閾値を超えた場合にアラートを発報。
- DBのスロークエリログを週次など定期的に確認し、応答遅延の兆候を早期発見する。
