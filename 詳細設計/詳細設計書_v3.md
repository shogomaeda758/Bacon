...
sequenceDiagram
    ...
    alt 入力数量が不正
        ...
        CC->>CS: addItem(productId, quantity, session)
        ...
        PR->>DB: findById(productId)SELECT * FROM PRODUCT WHERE product_id = ?
...
sequenceDiagram
    ...
    alt 入力数量が不正 (<=0 または NaN)
        ...
        CC->>CS: updateItem(itemId, quantity, session)
        CS->>S: updateItemQuantity(itemId, quantityDto.getQuantity(), session)
        S-->>CS: CartRespons
...
sequenceDiagram
    ...
    CC->>CS: removeItem(itemId, session)
    CS->>S: removeItemFromSession(session)
    S-->>CS: CartRespons
    CS->>CS: itemIdが存在するかチェック
    alt カートにitemIdがない
...
sequenceDiagram
    ...
    alt バリデーションエラー
        ...
        B->>CC: GET /api/cart
        CC->>CS: getCart(session)
        CS-->>CC: getCartFromSession(session)
...
sequenceDiagram
    ...
    alt ログイン済み (customerIdが存在)
        ...
        CUC->>S: getCustomerProfile(HttpSession session)
        S-->>CUC: customerId
        CUC->>CUS:getCustomer(customerId)
        CUS->>CR: getCustomerByID(customerId)
        CR->>DB: findById(customerId)
        DB-->>CR: Customer
        ...
        Note right of U: 空の注文情報入力フォームが表示
    end
```
### 3.2.11. 注文確定フロー（正常系）
