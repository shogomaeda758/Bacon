...
- テストケース名: カートに存在する商品と別の商品を追加する
- 前提条件:
  - カートに既存商品が存在する。
  - DBに追加対象の商品が存在する。
- 手順:
  1. HTTP POSTリクエストを /api/cart に送信する。リクエストボディに追加商品のIDと数量を含める。
- 入力データ:
  - リクエストボディ(JSON): {"productId": "追加商品ID", "quantity": "追加数量"}
  - セッション: 既存商品を含むカート情報
- 期待結果:
  1. HTTPステータスコード 200 OK が返却される。
  2. 新しい商品がカートに追加され、合計が更新されたカート情報がJSONで返却される。
